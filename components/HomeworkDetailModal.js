...
const HomeworkDetailModal = React.memo(({
  visible,
  onClose,
  homework,
  currentUserId,
  ...
  const canEdit = currentUserId === homework.created_by;
        ...
        >
          <View style={styles.messageWrapper}>
            {isEditing ? (
              <TextInput
                ...
                multiline
                onChangeText={(t) => onUpdate({ ...homework, description: t })}
              />
            ) : (
          ...
          <View style={[styles.metaCard, { backgroundColor: theme.colors.card, borderColor: theme.colors.cardBorder, borderWidth: 1 }]}>
              ...
              <View>
                ...
                {isEditing ? (
                  <TextInput
                    ...
                    onChangeText={(t) => onUpdate({ ...homework, subject: t })}
                  />
                ) : (
                  <Text style={[styles.metaValue, { color: theme.colors.text }]}>{homework.subject}</Text>
                )}
              </View>
            </View>
            ...
            <View style={styles.metaRow}>
              ...
              <View>
                ...
                <Text style={[styles.metaValue, { color: theme.colors.text }]}>{formatDate(homework.due_date)}</Text>
              </View>
            </View>

            {homework.created_by_user && (
              <>
                <View style={[styles.metaDivider, { backgroundColor: theme.colors.cardBorder }]} />
                <View style={styles.metaRow}>
                  <View style={[styles.metaIcon, { backgroundColor: theme.colors.primary + '10' }]}>
                  ...
                  <View>
                    ...
                    <Text style={[styles.metaValue, { color: theme.colors.text }]}>{homework.created_by_user.full_name}</Text>
                  </View>
                </View>
              </>
            )}

            {homework.lesson_plans && (
              <>
                <View style={[styles.metaDivider, { backgroundColor: theme.colors.cardBorder }]} />
                <View style={styles.metaRow}>
                  <View style={[styles.metaIcon, { backgroundColor: theme.colors.primary + '10' }]}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={[styles.metaValue, { color: theme.colors.text }]}>{homework.lesson_plans.title}</Text>
                    {homework.lesson_plans.objectives?.length > 0 && (
                      <Text style={{ fontSize: 11, color: theme.colors.placeholder, marginTop: 4 }}>
                        Focus: {homework.lesson_plans.objectives[0]}
                      </Text>
                    )}
                  </View>
                </View>
          ...
          {homework.resources?.length > 0 && (
            <View style={{ marginTop: 24 }}>
              <Text style={[styles.cardSectionLabel, { color: theme.colors.placeholder, marginBottom: 12, marginLeft: 4 }]}>ATTACHED MATERIALS ({homework.resources.length})</Text>
          ...
          {canEdit && onUpdate && onDelete && (
            <View style={[styles.modalButtonContainer, { marginTop: 24 }]}>
              <TouchableOpacity
                ...
                onPress={() => (isEditing ? onUpdate(homework) : setIsEditing(true))}
              >
